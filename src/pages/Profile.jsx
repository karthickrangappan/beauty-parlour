    ...
    <div className="min-h-screen bg-cream-50 pt-32 pb-24">
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
        ...
        >
          <div>
            <h2
              className="text-2xl text-neutral-800 font-light mb-2"
          ...
          <div className="flex flex-col space-y-4">
            <button
              ...
              className={`text-left text-sm uppercase tracking-widest font-medium transition-colors ${activeTab === "profile" ? "text-neutral-900" : "text-neutral-400 hover:text-gold-500"}`}
            >
              My Profile
            </button>
            <button
              onClick={() => setActiveTab("orders")}
              className={`text-left text-sm uppercase tracking-widest font-medium transition-colors ${activeTab === "orders" ? "text-neutral-900" : "text-neutral-400 hover:text-gold-500"}`}
            >
              Orders
            </button>
            <button
              onClick={() => setActiveTab("appointments")}
              className={`text-left text-sm uppercase tracking-widest font-medium transition-colors ${activeTab === "appointments" ? "text-neutral-900" : "text-neutral-400 hover:text-gold-500"}`}
            >
              Appointments
            </button>
            <button
              onClick={() => navigate("/wishlist")}
              className="text-left text-sm uppercase tracking-widest font-medium text-neutral-400 hover:text-gold-500 transition-colors"
            >
              Wishlist
            </button>
            ...
            {user?.role === "admin" && (
              <button
                ...
                className="text-left text-sm uppercase tracking-widest font-bold text-gold-500 hover:text-gold-600 transition-colors pt-4 mt-4 border-t border-neutral-200"
              >
                Admin Portal
              </button>
            )}
          ...
          <div className="relative z-10">
            ...
            {activeTab === "profile" && (
              <div className="space-y-12">
                <section>
                  <h3
                    className="text-xl text-neutral-800 font-light mb-6 border-b border-gold-300/30 pb-4"
                    style={{ fontFamily: "ui-serif, Georgia, serif" }}
                  >
                    Account Information
                  </h3>
            ...
            {activeTab === "orders" && (
              <div>
                <h3
                  className="text-xl text-neutral-800 font-light mb-8 border-b border-gold-300/30 pb-4"
                  style={{ fontFamily: "ui-serif, Georgia, serif" }}
                >
                  Order History
                </h3>
                {isLoading ? (
                  ...
                  <div className="text-center py-12 border border-dashed border-neutral-200">
                    ...
                    <p className="text-neutral-500 font-light font-serif italic mb-4">
                      You have not placed any orders yet.
                    </p>
                    <button
                      onClick={() => navigate("/shop")}
                      className="text-[10px] uppercase tracking-widest text-gold-500 hover:text-neutral-900 border-b border-gold-500 pb-1"
                    >
                      Discover Collection
                    </button>
                  </div>
                  ...
                  <div className="space-y-8">
                    {orders.map((order) => {
                      ...
                      return (
                        <div
                          key={order.id}
                          className="border border-gold-300/20 shadow-sm"
                        >
                          {/* Order Header */}
                                ...
                                <div className="flex items-center justify-between relative">
                                  ...
                                  {orderSteps.map((step, idx) => (
                                    ...
                                    >
                                      ...
                                      <span
                                        className={`absolute top-6 text-[10px] uppercase tracking-widest ${idx <= currentStep ? "text-neutral-800 font-bold" : "text-neutral-400 font-medium"} max-w-[60px] text-center`}
                                      >
                                        {step}
                                      </span>
                                    </div>
                              ...
                              {order.items?.map((item) => (
                                ...
                                >
                                  <img
                                    src={item.image}
                                    alt={item.name}
                                    className="w-12 h-16 object-cover border border-gold-300/10"
                                  />
                                  <div className="flex-1">
                                    <h4 className="text-xs uppercase tracking-wider text-neutral-800">
                                      {item.name}
                                    </h4>
                            ...
                            {order.status === "delivered" && (
                              <div className="mt-8 pt-6 border-t border-gold-300/10 text-right">
                                <button
                                  ...
                                  className="text-[10px] uppercase tracking-widest text-red-500 hover:text-red-700 flex items-center justify-end gap-1 w-full text-right"
                                >
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-6">
                    {appointments.map((appt) => (
                      <div
                        key={appt.id}
                        className="border border-gold-300/20 p-6 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center hover:shadow-md transition-shadow"
                      >
                        <div className="space-y-2 flex-1">
                          <div className="flex gap-2 items-center mb-1">
                            <span
                              className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 ${appt.status === "cancelled" ? "bg-red-100 text-red-600" : appt.status === "confirmed" ? "bg-green-100 text-green-700" : "bg-gold-100 text-gold-700"}`}
                            >
                              {appt.status}
                            </span>
                          ...
                          <p className="text-xs text-neutral-500 italic font-serif flex items-center gap-1">
                            ...
                            {new Date(appt.date).toLocaleDateString()} &bull;{" "}
                            {appt.time} &bull; {appt.duration}min
                          </p>
                          <p className="text-[10px] text-neutral-400 uppercase tracking-widest">
                            Specialist: {appt.staffName}
                          </p>
                        </div>

                        <div className="w-full md:w-auto text-right">
                          <p className="text-lg font-light text-neutral-800 mb-2">
