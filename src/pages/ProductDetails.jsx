...
const ProductDetails = () => {
  ...
  // Reset alreadyInCart warning when product changes
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-cream-50">
        <div className="text-center">
          ...
          <Link
            to="/shop"
            className="text-gold-500 underline uppercase tracking-widest text-xs"
          >
            Back to Shop
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-white pb-24">
      ...
      <div className="bg-cream-50 py-4 mt-16 md:mt-20">
        <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-10 flex flex-wrap items-center gap-2 text-[9px] md:text-[10px] uppercase tracking-widest text-neutral-400">
          ...
          <ChevronRight className="w-2.5 h-2.5 md:w-3 h-3" />
          <span className="text-neutral-900 font-semibold truncate max-w-[150px] md:max-w-none">
            {product.name}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mt-8 md:mt-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 lg:gap-24 items-start">
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            >
              <div className="flex items-center gap-3 md:gap-4">
                ...
                {product.rating > 0 && (
                  <div className="flex items-center gap-1">
                    <Star className="w-3.5 h-3.5 md:w-4 md:h-4 fill-gold-500 text-gold-500" />
                    <span className="text-xs md:text-sm font-medium text-neutral-900 mt-0.5">
                      {product.rating}
                    </span>
                  </div>
                )}
              ...
              <h1
                className="text-3xl sm:text-4xl md:text-5xl font-light text-neutral-900 leading-tight"
                style={{ fontFamily: "ui-serif, Georgia, serif" }}
              >
                {product.name}
              </h1>

              <p className="text-xl md:text-2xl font-light text-neutral-900">
                {fmtCurrency(product.price || 0)}
              </p>

              <p className="text-neutral-500 leading-relaxed text-sm md:text-lg max-w-xl">
                Elevate your daily ritual with the {product.name}. Carefully
                formulated to provide{" "}
                {(product.shortDesc || "exceptional results").toLowerCase()} for
                a truly luminous finish.
              </p>

              {isLowStock && (
                <p className="text-[10px] md:text-xs text-orange-600 uppercase tracking-widest font-bold">
                  Only {product.stock} left in stock
                </p>
              )}
              {isOutOfStock && (
                <p className="text-[10px] md:text-xs text-red-600 uppercase tracking-widest font-bold">
                  Out of stock
                </p>
              )}
            </motion.div>

            {/* Quantity, Wishlist & Buy Now */}
            <div className="space-y-4 pt-6 border-t border-neutral-100">
              <div className="flex items-center gap-4 md:gap-6">
                {/* Quantity selector — hidden if already in cart */}
                {!alreadyInCart && (
                ...
                {alreadyInCart && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-[11px] uppercase tracking-widest text-amber-700 font-bold underline underline-offset-2 hover:text-amber-900 transition"
                    >
                      View Cart →
                    </button>
                  </motion.div>
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-4 py-6 md:py-8 border-y border-neutral-100 mt-8 md:mt-12">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <span className="text-[8px] md:text-[10px] uppercase tracking-widest text-neutral-500 font-bold leading-tight">
                  100% Pure
                </span>
              </div>
              <div className="flex flex-col items-center text-center gap-2">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === "description" && (
                  <p>
                    Designed for the discerning individual, our {product.name}{" "}
                    delivers unparalleled results by utilizing high-performance
                    active ingredients. It works deeply into the surface of your
                    skin to provide{" "}
                    {(product.shortDesc || "exceptional results").toLowerCase()}{" "}
                    while maintaining its natural moisture barrier.
                  </p>
                )}
                {activeTab === "how to use" && (
                  <p>
                    Apply a small amount to clean skin twice daily. Massage
                    gently in upward circular motions until fully absorbed. For
                    best results, follow with our recommended serum pairing.
                  </p>
                )}
                {activeTab === "ingredients" && (
                  <p>
                    Aqua (Water), Glycerin, Squalane, Niacinamide,
                    Butyrospermum Parkii (Shea) Butter, Hyaluronic Acid,
                    Tocopherol (Vitamin E), Botanical Extracts, Organic
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-20 md:mt-32">
            <h2
              className="text-2xl md:text-3xl font-light mb-8 md:mb-12"
              style={{ fontFamily: "ui-serif, Georgia, serif" }}
            >
              You May Also Like
            </h2>
